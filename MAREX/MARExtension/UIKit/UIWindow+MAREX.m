...
- (UIImage * _Nonnull)mar_takeScreenshot {
    return [self mar_takeScreenshotAndSave:NO];
}

- (void)mar_takeScreenshotWithDelay:(CGFloat)delay save:(BOOL)save completion:(void (^ _Nullable)(UIImage *image))completion {
    dispatch_after(dispatch_time(DISPATCH_TIME_NOW, delay * NSEC_PER_SEC), dispatch_get_main_queue(), ^{
        completion([self mar_takeScreenshotAndSave:save]);
    });
}
...
- (void)mar_activateTouch {
    ...
    Method exchangedSendEvent = class_getInstanceMethod([self class], @selector(mar_exchangedSendEvent:));
...
- (void)mar_exchangedSendEvent:(UIEvent *)event {
    [self mar_handleEvent:event];
    [self mar_exchangedSendEvent:event];
}

- (void)mar_handleEvent:(UIEvent * _Nonnull)event {
    ...
    for (UITouch *touch in event.allTouches) {
        ...
        switch (phase) {
            case UITouchPhaseBegan: {
                UIImageView *touchImageView = [[UIImageView alloc] initWithImage:self.mar_touchImage];
...
- (UIImage * _Nonnull)mar_touchImage {
    ...
    if ([[UIScreen mainScreen] scale] == 2) {
        return [image mar_imageByResizeToSize:CGSizeMake((image.size.width / 5) * 3, (image.size.width / 5) * 3)];
//        return [image imageByScalingProportionallyToSize:CGSizeMake((image.size.width / 5) * 3, (image.size.width / 5) * 3)];
    } else {
        return [image mar_imageByResizeToSize:CGSizeMake((image.size.width / 3) * 2, (image.size.width / 3) * 2)];
