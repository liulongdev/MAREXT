...
- (NSData *)pMar_hmacDataUsingAlg:(CCHmacAlgorithm)alg withKey:(NSData *)key {
    size_t size;
    switch (alg) {
        case kCCHmacAlgMD5:     size = CC_MD5_DIGEST_LENGTH;    break;
        case kCCHmacAlgSHA1:    size = CC_SHA1_DIGEST_LENGTH;   break;
...
- (NSString *)mar_hmacMD5StringWithKey:(NSString *)key {
    return [self pMar_hmacStringUsingAlg:kCCHmacAlgMD5 withKey:key];
}

- (NSData *)mar_hmacMD5DataWithKey:(NSData *)key {
    return [self pMar_hmacDataUsingAlg:kCCHmacAlgMD5 withKey:key];
}

- (NSString *)mar_hmacSHA1StringWithKey:(NSString *)key {
    return [self pMar_hmacStringUsingAlg:kCCHmacAlgSHA1 withKey:key];
}

- (NSData *)mar_hmacSHA1DataWithKey:(NSData *)key {
    return [self pMar_hmacDataUsingAlg:kCCHmacAlgSHA1 withKey:key];
}

- (NSString *)mar_hmacSHA224StringWithKey:(NSString *)key {
    return [self pMar_hmacStringUsingAlg:kCCHmacAlgSHA224 withKey:key];
}

- (NSData *)mar_hmacSHA224DataWithKey:(NSData *)key {
    return [self pMar_hmacDataUsingAlg:kCCHmacAlgSHA224 withKey:key];
}

- (NSString *)mar_hmacSHA256StringWithKey:(NSString *)key {
    return [self pMar_hmacStringUsingAlg:kCCHmacAlgSHA256 withKey:key];
}

- (NSData *)mar_hmacSHA256DataWithKey:(NSData *)key {
    return [self pMar_hmacDataUsingAlg:kCCHmacAlgSHA256 withKey:key];
}

- (NSString *)mar_hmacSHA384StringWithKey:(NSString *)key {
    return [self pMar_hmacStringUsingAlg:kCCHmacAlgSHA384 withKey:key];
}

- (NSData *)mar_hmacSHA384DataWithKey:(NSData *)key {
    return [self pMar_hmacDataUsingAlg:kCCHmacAlgSHA384 withKey:key];
}

- (NSString *)mar_hmacSHA512StringWithKey:(NSString *)key {
    return [self pMar_hmacStringUsingAlg:kCCHmacAlgSHA512 withKey:key];
}

- (NSData *)mar_hmacSHA512DataWithKey:(NSData *)key {
    return [self pMar_hmacDataUsingAlg:kCCHmacAlgSHA512 withKey:key];
}
...
- (NSData *)mar_aes256EncryptWithKey:(NSData *)key iv:(NSData *)iv
{
...
- (NSData *)mar_aes256DecryptWithkey:(NSData *)key iv:(NSData *)iv {
    if (key.length != 16 && key.length != 24 && key.length != 32) {
        return nil;
    }
    if (iv.length != 16 && iv.length != 0) {
...
+ (NSData *)mar_dataWithHexString:(NSString *)hexStr
{
...
static const char mar_base64EncodingTable[64]
= "ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789+/";
static const short mar_base64DecodingTable[256] = {
...
- (NSString *)mar_base64EncodedString
{
    ...
    for (i = 0; i < length; i += 3) {
        ...
        output[index + 0] = mar_base64EncodingTable[(value >> 18) & 0x3F];
        output[index + 1] = mar_base64EncodingTable[(value >> 12) & 0x3F];
        output[index + 2] = ((i + 1) < length) ? mar_base64EncodingTable[(value >> 6) & 0x3F] : '=';
        output[index + 3] = ((i + 2) < length) ? mar_base64EncodingTable[(value >> 0) & 0x3F] : '=';
    }
...
+ (NSData *)mar_dataWithBase64EncodedString:(NSString *)base64EncodedString
{
    ...
    while (inputPoint < length) {
        ...
        output[outputPoint++] = (mar_base64DecodingTable[i0] << 2)
        | (mar_base64DecodingTable[i1] >> 4);
        if (outputPoint < outputLength) {
            output[outputPoint++] = ((mar_base64DecodingTable[i1] & 0xf) << 4)
            | (mar_base64DecodingTable[i2] >> 2);
        }
        if (outputPoint < outputLength) {
            output[outputPoint++] = ((mar_base64DecodingTable[i2] & 0x3) << 6)
            | mar_base64DecodingTable[i3];
        }
    }
    return data;
}
...
- (NSData *)mar_gzipDeflate
{
    ...
    if (deflateInit2(&strm, Z_DEFAULT_COMPRESSION, Z_DEFLATED, (15 + 16), 7, Z_DEFAULT_STRATEGY) != Z_OK) {
        return nil;
    }
...
+ (NSData *)mar_dataNamed:(NSString *)name {
